...
# Loaded Data and Checks. 
This is a check to enure that the data is loaded and works in R. 
...
library(ggplot2)
# get the file names
Nw_names <- list.files(path="FlankerData")
...
for(i in Nw_names){
  ...
  a_data <- rbind(a_data,one_subject)
}
```


```{r}
center_letter <- unlist(lapply(strsplit(a_data$stimulus,""),
                           FUN=function(x)unlist(x)[2]))
a_data <- cbind(a_data,center_letter)
a_data <- a_data %>%
...
```{r}
previous_congruency <- c(0,a_data$congruency[1:(length(a_data$congruency)-1)])
sequence_subjects <- a_data %>%
